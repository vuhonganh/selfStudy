...
pairs, where each pair is a tuple and the locations are a tuple.

NOTE: we choose this format for easy to transfer to a dictionary (dict(puzzle))

Here is the initial state for the problem above in this format:
"""
...
def solve_parking_puzzle(start, N=N):
    ...
    return shortest_path_search(start, parking_successors, is_goal_parking)


def is_goal_parking(state):
    board = dict(state)
    return board['@'][0] in board['*']



def update_state(old_state, car_name, new_idx):
    """Given a parking in an old_state. Update this old_state by
    placing car_name in new_idx. Note that state is in tuple"""
    state_in_dict = dict(old_state)
    state_in_dict[car_name] = new_idx
    return tuple(state_in_dict.items())


def parking_successors(state):
    """Return a dict {state:action} reachable from this state"""
    res = {}
    occupied = set(p for (cname, idx) in state for p in idx if cname != '@')
    for (car_name, idx) in state:
        if car_name != '@' and car_name != ' |':
            incr = idx[1] - idx[0]  # the possible increment of this car
            for k in (-1, 1):  # moving in 2 opposite directions:
                distance = 0
                while True:
                    distance += k * incr
                    new_pos = idx[0] + distance if k == -1 else idx[-1] + distance  # find the head of movement
                    if new_pos < 0 or new_pos >= N*N or new_pos in occupied:
                        break
                    new_idx = tuple(map(operator.add, idx, [distance] * len(idx)))
                    res[update_state(state, car_name, new_idx)] = (car_name, distance)
    return res
...
def grid(cars, N=N):
    ...
    for car in cars:
        res.append(car)
    # now adding the walls:
    wall_up = locs(0, N)
    wall_left = locs(N, N - 1, N)
    wall_down = locs(N * (N-1), N - 1)
    buffer_wall_r = list(locs(2 * N - 1, N - 1, N))
    buffer_wall_r.remove(goal_idx[0])
    wall_right = tuple(buffer_wall_r)
...
def show_solve(puzzle, N=N):
    paths = solve_parking_puzzle(puzzle, N)
