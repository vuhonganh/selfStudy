...
def replacements(card):
    "Return a list of all possible replacements for a card. If it is not wild card return itself as a list"
...
def best_wild_hand(hand):
    "Try all value for jokers and return the best 5-card hand. Jokers will be treated as wild cards which can take any rank or suit of the same color."
    hands = set(best_hand(hand) for hand in itertools.product(*map(replacements, hand)))
    #Note: the * before map is for unpacking elements
...
def deal(numhands, n=5, deck=mydeck):
    #first shuffle the deck and return each hand the nb of card n 
    random.shuffle(deck)
