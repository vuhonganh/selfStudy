...
# NOTE: this version is from the course instructor: j will be row and i will be column of board
...
LETTERS_LOWER = list('ABCDEFGHIJKLMNOPQRSTUVWXYZ'.lower())
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    if pre.upper() in PREFIXES:
        ...
        if is_letter(sq):
            ...
            for L in hand:  # NOTE: need to take care of L = blank title _
                if L in possibilities:
                    add_suffixes(hand.replace(L, '', 1), pre + L, start, row, results)
                elif L == '_':  # replace blank title L by any letter
                    for blank_title in LETTERS:
                        add_suffixes(hand.replace(L, '', 1), pre + blank_title.lower(), start, row, results)
    return results
...
def find_prefixes(hand, pre='', results=None):
    ...
    if pre == '': prev_hand, prev_results = hand.upper(), results
...
def cross_word_score(board, L, pos, direction):
    ...
    # if new_word == 'tEF':
    #     print 'new word tEF is found in cross word score'
    #     print 'word is found by find_cross_word is: word = %s' % word
    return calculate_score(board, (i, j2), DOWN, L, new_word)
...
def is_letters(square):
    ...
    return (square in LETTERS or square in LETTERS_LOWER)


def test():
    print 'File: blank_titles.py'
    def ok(hand, n, s, d, w):

        # result = (score, pos, dir, w)
    ...
    show_best('__CEHKN', a_board())

    # assert ok('__CEHKN', 61, (9, 1), (1, 0), 'KiCk')  # NOTE: seems WRONG
    assert ok('__CEHKN', 67, (8, 1), (1, 0), 'tHiCK')
    return 'tests pass'
