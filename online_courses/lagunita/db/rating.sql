...
from (select avg(avgRating) as avgRatingAll from 
  (select avg(stars) as avgRating
  from Movie join Rating using (mID)
  where year < 1980
  group by title, year) filmsBf1980) S1, 
  (select avg(avgRating) as avgRatingAll from 
  (select avg(stars) as avgRating
  from Movie join Rating using (mID)
  where year > 1980
  group by title, year) filmsAf1980) S2;
