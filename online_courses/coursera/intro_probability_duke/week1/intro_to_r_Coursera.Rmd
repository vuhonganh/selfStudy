...
dim(present)  # returns nb row * nb cols, nb cols will be nb of variables
```
...
present <- present %>% mutate(total = boys + girls)  # add total variable
present <- present %>% mutate(prop_boys = boys / total)  # compute prob_boys
...
present <- present %>% mutate(more_boys = boys > girls)  # they are all TRUE
```
...
present <- present %>% mutate(prop_boy_girl = boys / girls)  # compute prop_boy_girl
...
```{r most-total-births}
present <-arrange(present, desc(total))  # 2007 is the answer
```
