...
```{r sfo-feb-flights}
dim(sfo_feb_flights)  # code to create sfo_feb_flights are already shown above
# result is 68 16 --> 68
```
...
ggplot(data = sfo_feb_flights, aes(x = arr_delay)) + geom_histogram(binwidth = 15)  # C is false
...
sfo_feb_flights %>% 
  group_by(carrier) %>% 
    summarise(mean_ad = mean(arr_delay), IQR_ad = IQR(arr_delay), n = n())
...
nycflights <- nycflights %>% mutate(avg_speed = 60 * distance / air_time ) 
...
ggplot(nycflights, aes(x = avg_speed, y = distance)) + geom_point()  # C is true
```
...
nycflights <- nycflights %>% mutate(del_type = ifelse(dep_delay <= 5, "on time", "delayed") )

# create a new data called dep_delayed which has only 2 vars del_type (delayed) and arr_type
dep_delayed <- nycflights %>% select(del_type, arr_type) %>% filter(del_type == "delayed")
# create new data called dd_aot: dep_delayed but arrive on time
dd_aot <- dep_delayed %>% filter(arr_type == "on time")
# percentage is:
dim(dd_aot)[1] / dim(dep_delayed)[1]
```