...
```{r perc-atheist-us12}
us12 %>% filter(response == "atheist") %>% summarise(n()) -> nb_atheist
nb_atheist / (us12 %>% summarise(n()))
```
...
Independence: seem ok, 50000 < 10% nb of world population --> OK
Sample Size / Skew: >10 failures and >10 success --> OK
...
Briefly, It's the error of calculation since every calculation or mesure has error.
...
info_stats = inference(y = response, data = us12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
info_stats$ME
```
...
atheism %>% filter(nationality == "Canada", year == "2012") -> can12
inference(response, data = can12, statistic = "proportion", success = "atheist", type = "ci", method = "theoretical")
atheism %>% filter(nationality == "France", year == "2012") -> fra12
inference(response, data = fra12, statistic = "proportion", success = "atheist", type = "ci", method = "theoretical")
```
...
inference(y = response, x = factor(year), data = spain, type = "ht", null = 0, alternative = "twosided", statistic = "proportion", success = "atheist", method = "theoretical")
```

Then answer is **B** False. Because the p-value is high (39.7%) hence we don't reject null hypothesis that there is no change.
...
inference(y = response, x = factor(year), data = us, type = "ht", null = 0, alternative = "twosided", statistic = "proportion", success = "atheist", method = "theoretical")
```

Answer is **A** True. Because p-value is nearly 0 hence we reject the null hypothesis.
...
```{r type1}
list_countries = c("Spain", "United States", "Japan", "Czech Republic", "France", "Korea, Rep (South)", "Germany", "Netherlands", "Austria", "Iceland", "Canada", "Switzerland", "Italy", "Argentina", "Russian Federation", "Finland", "Moldova", "Poland", "South Africa", "Bosnia and Herzegovina", "Malaysia", "Bulgaria")
count = 0
for (country in list_countries)
{
  info_country = atheism %>% filter(nationality == country)
  res_stats = inference(y = response, x = factor(year), data = info_country, type = "ht", null = 0, alternative = "twosided", statistic = "proportion", success = "atheist", method = "theoretical", verbose = FALSE)
  if (res_stats$p_value < 0.05)
  {
    count = count + 1
  }
}
...
Answer is UNKNOWN. I do not understand the question. However, i would choose 0.05 * 39 = 1.95 countries.
...
Answer is **D** At least 9604 people.
