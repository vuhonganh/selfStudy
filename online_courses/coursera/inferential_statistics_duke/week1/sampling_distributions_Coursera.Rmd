...
B is false: it should be the median, not the mean.
...
Of course C. 1000 is the correct answer because we get closer to the whole population.
...
The plot is shown above. There are 15000 elements `x_bar` in `sample_means50`. 
```{r sampling-dist}
sample_means50 %>% summarise(mean_x_bar = mean(x_bar), SE_x_bar = sd(x_bar))
```
...
ames %>% rep_sample_n(size = 10, replace = TRUE, reps = 25) %>% summarise(x_bar = mean(area)) -> sample_means_small
```
...
Of course there are 25 elements `x_bar` in the `sample_means_small` variable. For each small sample, 10 elements are randomly selected from the population and `x_bar` is their average.

```{r sample-means-small}
str(sample_means_small)
sample_means_small %>% summarise(means_small_mu = mean(x_bar), means_small_se = sd(x_bar))
```
...
Answer is **A** as explained above.
...
```{r price-sample}
ames %>% sample_n(size = 50, replace = TRUE) %>% summarise(mean_price = mean(price))
```
...
ames %>% rep_sample_n(size = 50, reps = 5000, replace = TRUE) %>% summarise(x_bar_price = mean(price)) -> sample_means50
ggplot(sample_means50, aes(x = x_bar_price)) + geom_histogram(binwidth = 2000)
sample_means50 %>% summarise(mean_price50 = mean(x_bar_price), se_price50 = sd(x_bar_price)) -> vals50
vals50
```
...
# type your code for this Exercise here, and Run Document
ames %>% rep_sample_n(size = 150, reps = 5000, replace = TRUE) %>% summarise(x_bar_price = mean(price)) -> sample_means150
ggplot(sample_means150, aes(x = x_bar_price)) + geom_histogram(binwidth = 2000)
vals150 <-sample_means150 %>% summarise(mean_price150 = mean(x_bar_price), se_price150 = sd(x_bar_price))
vals150
```

The `sample_means150` (standard error = `r round(vals150$se_price150)`) is less variable than `sample_means50` (standard error = `r round(vals50$se_price50)`).
...
Answer is **A** as explained above. The larger sample size will have less variability.
