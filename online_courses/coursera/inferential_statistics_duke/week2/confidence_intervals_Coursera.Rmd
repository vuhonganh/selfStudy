...
Based on the histogram, the typical zise is in the range 1000 - 1500 (squared unit) as size of houses mainly fall in this range.
...
samp %>%
  summarise(lower = mean(area) - z_star_95 * (sd(area) / sqrt(n)),
            upper = mean(area) + z_star_95 * (sd(area) / sqrt(n))) -> CI95
CI95
```
...
Answer is **C**: The sample distribution must be nearly normal. The sampling distribution will be normal by the Central Limit Theorem. However, we do not have any requirement that each sample distribution must be nearly normal.
...
Answer is **B**. Answer **A** is wrong because the true average is fixed, hence there is no 95% of the time.
...
CI95$lower <= params$mu && params$mu <= CI95$upper
```
...
Answer is **C** as this is property of 95% confidence intervals.
...
ggplot(data = ci_data, aes(x = ci_bounds, y = ci_id, 
                           group = ci_id, color = capture_mu)) +  # very important group = ci_id
...
94% of confidence intervals include the true population mean (47/50).
...
qnorm(0.995) -> z_star_99
z_star_99
```
Answer is **E** 2.58 because critical value  equals $Z_{\alpha/2}$ where $\alpha$ is the significance level.
...
```{r plot-99-perc-cis}
# type your code for the Exercise here, and Knit
ci <- ci %>% mutate(lower99 = lower - (z_star_99 - z_star_95)*(upper - lower)/(2*z_star_95))
ci <- ci %>% mutate(upper99 = upper + (z_star_99 - z_star_95)*(upper - lower)/(2*z_star_95))
ci <- ci %>%
  mutate(capture_mu99 = ifelse(lower99 < params$mu & upper99 > params$mu, "yes", "no"))

ci_data99 = data.frame(ci_id = c(1:50, 1:50),
                      ci_bounds = c(ci$lower99, ci$upper99),
                      capture_mu = c(ci$capture_mu99, ci$capture_mu99))
