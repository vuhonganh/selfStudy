...
Answer is **C** 27 based on the summary above.
...
Answer is **C** Both distribution are extremely right skewed. This is false because smoker distribution is slightly left skewed.
...
The conclusion is we can reject null hypothesis for a significance level $\alpha$ = 5% because p-value is smaller than this.
...
Answer is **D**. We can see that the confidence interval does not contain null value 0. Hence it means that we should reject null hypothesis. 
...
The 90% confidence interval is included in 99% confidence interval.
...
p_value is 0.17 which is bigger than significance level 5%. So we don't reject H0. 

7. Now, a non-inference task: Determine the age cutoff for younger and mature 
mothers. Use a method of your choice, and explain how your method works.
```{r cutoff-mature}
# type your code for Question 7 here, and Knit
ggplot(data = nc, aes(x = factor(mature), y = mage, color = mature)) + geom_line()
```

As shown in the picture, we can see the cutoff value is about 35 years old.
...
Here we consider the relation between race and weight of the baby. The null hypothese is the weight is the same for both white moms and non-white moms. The alternative hypothese is there is a difference.
```{r pick-your-own}
inference(y = weight, x = whitemom, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
```

The result shows that there is a difference. The p-value is really small so that we can reject H0.
